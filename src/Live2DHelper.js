...
function Live2DHelper(setting)
{
    ...
    this.live2DMgr = new LAppLive2DManager(this.canvas);
        ...
        this.canvas.addEventListener("touchstart", this.mouseEvent.bind(this), false);
        this.canvas.addEventListener("touchend", this.mouseEvent.bind(this), false);
        this.canvas.addEventListener("touchmove", this.mouseEvent.bind(this), false);
...
{      
    var ax=event.clientX/window.innerWidth;
    var ay=event.clientY/window.innerHeight;
//    var rect = event.target.getBoundingClientRect();
    var bx=this.canvas.getAttribute("width");
    var by=this.canvas.getAttribute("height");
    var sx = this.transformScreenX(bx*ax);
//    var sx = this.transformScreenX(event.clientX - rect.left);
    var sy = this.transformScreenY(by*ay);
//    var sy = this.transformScreenY(event.clientY - rect.top);
    var vx = this.transformViewX(bx*ax);
//    var vx = this.transformViewX(event.clientX - rect.left);
    var vy = this.transformViewY(by*ay);
